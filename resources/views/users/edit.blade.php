                        ...
                        <div class="mb-3">
                            ...
                            <select name="classroom" class="form-select">
                                ...
                                @foreach ($classrooms as $classroom)
                                    <option value="{{ $classroom->id }}" {{ ($user->classroom_id == $classroom->id) ? 'selected' : ''}}>{{ $classroom->title }}</option>
                                @endforeach
                            </select>
                            @error('classroom')
                                <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="major" class="form-select">
                                ...
                                @foreach ($majors as $major)
                                    <option value="{{ $major->id  }}" {{ ($user->major_id == $major->id) ? 'selected' : ''}}>{{ $major->title }}</option>
                                @endforeach
                            </select>
                            @error('major')
                                <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-3">
                                <div class="form-check">
                                  ...
                                  <div class="form-check">
                                    <input class="form-check-input" type="radio" name="gender" value="Female" id="flexRadioDefault2" {{ ($user->gender == 'Female') ? 'checked' : ''}}>
